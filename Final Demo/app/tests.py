...
from .views import home, register, login_view, profile, profile_edit, fuel_history, fuel_quote, get_price_and_total
...
class PriceModuleTest(TestCase):
    ...
    def test_price_module_code(self):
        # testing a get request to fuel_history view
        request = self.factory.get(self.url)
        request.user = self.user
        response = fuel_quote(request)
    ...
    def test_price_module_post(self):
        request = self.factory.post(self.url, data=json.dumps(self.data),
                                    content_type='application/json')
        request.user = self.user
        response = get_price_and_total(request)
